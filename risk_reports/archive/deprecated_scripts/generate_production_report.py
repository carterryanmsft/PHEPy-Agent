...
Embeds Kusto query, executes it, and generates the HTML report
"""
import subprocess
import json
import csv
...
def execute_kusto_query():
    """Execute the Kusto query using the MCP tool"""
    print("Executing Kusto query for 131 production cases...")
    
    query = """// IC/MCS High-Risk Case Report with Risk Scoring
...
| order by RiskScore desc, DaysOpen desc"""
    ...
    with open('temp_query.kql', 'w', encoding='utf-8') as f:
        f.write(query)
    
    # Note: This script should be called from a workflow that has the Kusto result
    # For now, returning None to indicate manual intervention needed
    return None

def convert_json_to_csv(kusto_json, output_csv):
    """Convert Kusto JSON result to CSV"""
    cases = kusto_json.get('data', [])
    
    if not cases:
        print("ERROR: No cases in Kusto result")
        return False
    
    print(f"Converting {len(cases)} cases to CSV...")
...
def generate_report(csv_file, report_file, icm_file):
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print("ERROR: This script needs the Kusto JSON result")
        print("Usage: python generate_production_report.py <kusto_result.json>")
        print("\nOr use the Kusto MCP tool to execute the query and save results")
        return 1
    
    # Convert to CSV
    csv_file = "data/production_cases_131.csv"
    if not convert_json_to_csv(kusto_result, csv_file):
        return 1
    ...
    if generate_report(csv_file, report_file, icm_file):
