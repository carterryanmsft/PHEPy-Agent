# Automated IC/MCS Production Report Generator
# Executes Kusto query, saves result, and generates HTML report

$ErrorActionPreference = "Stop"
$ProgressPreference = "SilentlyContinue"

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "IC/MCS Production Report Generator" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Step 1: Execute Kusto query and save to temp JSON
Write-Host "Step 1: Executing Kusto query..." -ForegroundColor Yellow
...
| extend RiskScore = 
    case(DaysOpen > 180, 40, DaysOpen > 120, 35, DaysOpen > 90, 30, DaysOpen > 60, 25, DaysOpen > 30, 20, 10)
    + case(OwnershipCount > 20, 20, OwnershipCount > 10, 15, OwnershipCount > 5, 10, OwnershipCount > 2, 5, 0)
    + case(TransferCount > 20, 15, TransferCount > 10, 12, TransferCount > 5, 8, TransferCount > 2, 4, 0)
    + case(DaysSinceUpdate > 30, 15, DaysSinceUpdate > 21, 12, DaysSinceUpdate > 14, 8, DaysSinceUpdate > 7, 4, 0)
    + case(ReopenCount >= 3, 10, ReopenCount == 2, 7, ReopenCount == 1, 5, 0)
    + case(isnotempty(RelatedICM_Id), 10, 0)
    + case(ServiceRequestCurrentSeverity in ("1", "A", "Critical"), 5, ServiceRequestCurrentSeverity in ("2", "B", "High"), 3, 0)
    + case(IsCritSit == "Yes", 10, 0)
| extend RiskScore = iff(RiskScore > 100, 100, RiskScore)
| extend RiskLevel = case(RiskScore >= 80, "Critical", RiskScore >= 60, "High", RiskScore >= 40, "Medium", "Low")
| extend AgeCategory = case(DaysOpen > 180, "Critical (>180 days)", DaysOpen > 120, "Very High (>120 days)", DaysOpen > 90, "High (>90 days)", DaysOpen > 60, "Elevated (>60 days)", DaysOpen > 30, "Moderate (>30 days)", "Recent (20-30 days)")
| extend HasICM = iff(isnotempty(RelatedICM_Id), "Yes", "No")
| extend Summary = strcat("Case is ", tostring(toint(DaysOpen)), " days old, status is ", ServiceRequestStatus, ", ICM present: ", HasICM, ". ", case(OwnershipCount > 10, strcat("Extremely high ownership (", OwnershipCount, ") and transfer (", TransferCount, ") counts indicate severe instability."), OwnershipCount > 5, strcat("High ownership (", OwnershipCount, ") and transfer (", TransferCount, ") counts increase risk."), OwnershipCount > 2, strcat("Ownership (", OwnershipCount, ") and transfer (", TransferCount, ") counts add to risk."), "No major additional risk factors."))
| project ModifiedDate, ServiceRequestNumber, CaseUrl = CaseUri, ServiceRequestStatus, ServiceRequestState, ProductName, DerivedProductName, TenantId, TopParentName, TpAccountName, SAPPath, Program, PHE, CLE, AgentAlias, ManagerEmail, ServiceRequestCurrentSeverity, CreatedTime, DaysOpen, AgeCategory, OwnershipCount, TransferCount, ReopenCount, RelatedICM_Id, HasICM, IsCritSit, RiskScore, RiskLevel, Summary, QueueName, TPID
| order by RiskScore desc, DaysOpen desc
"@

# Note: The Kusto MCP tool needs to be called interactively by GitHub Copilot
Write-Host "  NOTE: This script template shows the automation workflow." -ForegroundColor Cyan
Write-Host "  The actual Kusto query must be executed via the MCP tool." -ForegroundColor Cyan
Write-Host ""
Write-Host "Manual steps required:" -ForegroundColor Yellow
Write-Host "  1. Ask Copilot to execute the Kusto query using mcp_kusto-mcp-ser_execute_query" -ForegroundColor White
Write-Host "  2. Copilot will save the JSON result to data/kusto_result.json" -ForegroundColor White
Write-Host "  3. Then run: python generate_production_report.py data/kusto_result.json" -ForegroundColor White
Write-Host ""
Write-Host "Query saved to: queries/ic_mcs_risk_report.kql" -ForegroundColor Green
