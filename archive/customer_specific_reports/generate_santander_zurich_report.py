#!/usr/bin/env python3
"""
Generate Combined Santander & Zurich Purview Case History Report
"""
import json
...
# Load case data
with open("data/purview_case_history_Santander_Zurich_6mo.json", "r", encoding="utf-8") as f:
    content = f.read()
    # The file starts with "Query results: " so we need to parse it
...
# Tenant breakdown
santander_cases = [c for c in cases if c["TenantId"] == "35595a02-4d6d-44ac-99e1-f9ab4cd872db"]
zurich_cases = [c for c in cases if c["TenantId"] == "473672ba-cd07-4371-a2ae-788b4c61840e"]

santander_open = len([c for c in santander_cases if c["ServiceRequestState"] == "Open"])
zurich_open = len([c for c in zurich_cases if c["ServiceRequestState"] == "Open"])

# Severity breakdown
sev_a = len([c for c in cases if c["ServiceRequestCurrentSeverity"] == "A"])
sev_b = len([c for c in cases if c["ServiceRequestCurrentSeverity"] == "B"])
sev_c = len([c for c in cases if c["ServiceRequestCurrentSeverity"] == "C"])
...
<head>
    ...
    <title>Santander & Zurich Purview Case History - 6 Month Analysis</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>📊 Santander & Zurich Purview Case History</h1>
            <div class="subtitle">Combined Analysis: Grupo Santander & Zürich Schweiz</div>
            <div class="subtitle">6 Month Case Review</div>
            <div class="meta">
                ...
                &nbsp;&nbsp;• Grupo Santander (35595a02-4d6d-44ac-99e1-f9ab4cd872db)<br>
                &nbsp;&nbsp;• Zürich Schweiz (473672ba-cd07-4371-a2ae-788b4c61840e)<br>
        ...
        <div class="content">
            ...
            <div class="summary-cards">
                ...
                <div class="card santander">
                    <h3>Santander</h3>
                    <div class="value">{len(santander_cases)}</div>
                    <div class="label">{santander_open} open cases</div>
                </div>
                <div class="card zurich">
                    <h3>Zurich</h3>
                    <div class="value">{len(zurich_cases)}</div>
                    <div class="label">{zurich_open} open cases</div>
                </div>
...
for case in top_oldest:
    title = case["Title"][:70] + "..." if len(case["Title"]) > 70 else case["Title"]
    ...
    customer = "Santander" if case["TenantId"] == "35595a02-4d6d-44ac-99e1-f9ab4cd872db" else "Zurich"
    customer_class = customer.lower()
    
    html += f"""                        <tr>
                            <td><a href="{case['CaseUri']}" class="case-link" target="_blank">{case['ServiceRequestNumber']}</a></td>
                            <td><span class="badge {customer_class}">{customer}</span></td>
                            <td>{title}</td>
...
# Santander stats
sant_open = [c for c in santander_cases if c["ServiceRequestState"] == "Open"]
sant_avg_days = round(sum(c["DaysOpen"] for c in sant_open) / len(sant_open), 1) if sant_open else 0
sant_icm = len([c for c in santander_cases if c["RelatedICM_Id"] and c["RelatedICM_Id"] != ""])

html += f"""                        <tr>
                            <td><strong>Grupo Santander</strong></td>
                            <td>{len(santander_cases)}</td>
                            <td>{santander_open}</td>
                            <td>{len(santander_cases) - santander_open}</td>
                            <td>{sant_avg_days}</td>
                            <td>{sant_icm}</td>
                        </tr>
"""
...
# Zurich stats
zur_open = [c for c in zurich_cases if c["ServiceRequestState"] == "Open"]
zur_avg_days = round(sum(c["DaysOpen"] for c in zur_open) / len(zur_open), 1) if zur_open else 0
zur_icm = len([c for c in zurich_cases if c["RelatedICM_Id"] and c["RelatedICM_Id"] != ""])

html += f"""                        <tr>
                            <td><strong>Zürich Schweiz</strong></td>
                            <td>{len(zurich_cases)}</td>
                            <td>{zurich_open}</td>
                            <td>{len(zurich_cases) - zurich_open}</td>
                            <td>{zur_avg_days}</td>
                            <td>{zur_icm}</td>
                        </tr>
                    </tbody>
...
# Save report
output_path = Path("risk_reports/Santander_Zurich_Purview_Case_History_6mo.htm")
...
print(f"\n🏢 By Customer:")
print(f"   Santander: {len(santander_cases)} cases ({santander_open} open)")
print(f"   Zurich: {len(zurich_cases)} cases ({zurich_open} open)")
